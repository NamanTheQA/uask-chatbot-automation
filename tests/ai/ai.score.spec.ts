...
test.describe('AI Batch Quality Scoring', () => {
  ...
  test('LLM Validation - Mediocre Response (70-90 range)', async () => {
    ...
    console.log(`\nMEDIOCRE RESPONSE DEMO - LLM Validation:`);
    ...
    expect(llmValidation.llmValidation?.relevanceScore).toBeGreaterThanOrEqual(60);
    expect(llmValidation.llmValidation?.appropriatenessScore).toBeGreaterThanOrEqual(60);
  ...
  test('AI response consistency scoring (multi-run stability)', async () => {
    ...
    // Get second response for same question
    await chat.sendMessage(prompt.text);
    await chat.waitForAIResponse();
    const response2 = await chat.getLastAIResponse();

    // Traditional consistency calculation
    ...
    console.log(`Traditional Consistency Score: ${consistency.consistencyScore}%`);
    console.log(`Issues: ${consistency.reasons.join(', ')}`);

    // LLM validation for consistency check
    const llmValidation1 = await validateWithOpenRouter(
      prompt.text,
      response1,
      OPENROUTER_API_KEY,
      FREE_MODELS.GEMINI_PRO
    ...
    const llmValidation2 = await validateWithOpenRouter(
      prompt.text,
      response2,
      OPENROUTER_API_KEY,
      FREE_MODELS.GEMINI_PRO
    ...
    console.log(`\nLLM Validation - Response 1:`);
    console.log(`  Relevance: ${llmValidation1.llmValidation?.relevanceScore}/100`);
    console.log(`  Appropriateness: ${llmValidation1.llmValidation?.appropriatenessScore}/100`);

    console.log(`\nLLM Validation - Response 2:`);
    console.log(`  Relevance: ${llmValidation2.llmValidation?.relevanceScore}/100`);
    console.log(`  Appropriateness: ${llmValidation2.llmValidation?.appropriatenessScore}/100`);

    // Check consistency score
    expect(consistency.consistencyScore).toBeGreaterThanOrEqual(60);
    
    // Both responses should meet quality thresholds
    expect(llmValidation1.llmValidation?.relevanceScore).toBeGreaterThanOrEqual(70);
    expect(llmValidation2.llmValidation?.relevanceScore).toBeGreaterThanOrEqual(70);
    ...
    const reportData = {
      ...
      llm: {
        response1: llmValidation1.llmValidation,
        response2: llmValidation2.llmValidation,
      },
      timestamp: new Date().toISOString()
    };

    saveReport('ai-consistency-report.json', reportData);
